...
rpm:
	...
	sed "s/version.*[0-9]\.[0-9]*/version 0.54/"\
	 gocr.spec >$(TRPM)/SPECS/gocr.spec
	cp ../gocr-0.54.tar.gz $(TRPM)/SOURCES
	#rpmbuild -ba --nobuild gocr.spec  # bin+src-rpm
	(cd $(TRPM)/SPECS; rpmbuild -bb gocr.spec)
	rpm -qil -p $(TRPM)/RPMS/i?86/gocr-0.54-*.i?86.rpm
